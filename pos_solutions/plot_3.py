...
name_x = "3part_mov_x.csv"
name_y = "3part_mov_y.csv"
#name_z = "2part_mov_z.csv"
data_x = loadtxt(name_x,comments="#",delimiter=",",usecols=(0,1,2))
data_y = loadtxt(name_y,comments="#",delimiter=",",usecols=(0,1,2))
...
for i in range(0,n_timesteps,k):  
    ...
    #stepsize=0.5;
    #grav_constant=0.1;
    ax.set_title("3 Particles: N=1000000, dT=0.5, G=0.1")
    ax.set_xlim(-2000,2000)
    ax.set_ylim(-500,1500)
    print(data_x[i,0],data_y[i,0])
    print(data_x[i,1],data_y[i,1])
    ax.plot(data_x[i,0],data_y[i,0],color="blue",marker=".",markersize=10,linewidth=0,linestyle="-",label=r"$M_1=1$")
    ax.plot(data_x[i,1],data_y[i,1],color="green",marker=".",markersize=20,linewidth=0,linestyle="-",label=r"$M_2=10$")
    ax.plot(data_x[i,2],data_y[i,2],color="red",marker=".",markersize=30,linewidth=0,linestyle="-",label=r"$M_3=100$")
...
os.system("mencoder 'mf://_tmp*.png' -mf type=png:fps=10 -ovc lavc -lavcopts vcodec=wmv2 -oac copy -o 3particles.mpg")
os.system("rm *.png")
